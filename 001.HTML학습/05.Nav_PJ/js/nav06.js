...
/**************************************************** 
    ...
    -> 결과로 변경된 배열이 리턴됨
    -> 새로 만들어진 배열데이터를 현재 자리에 그대로 출력하고자 할 때
        배열메서드 join(구분자)를 이용하며 배열을 하나의 문자형 데이터로 만들어 주면 편리하다

        예)  const aa = ['치킨','피자']
        `<div>${aa.map(val=>`<h2>${val}</h2>`).join('')}</div>`
        - 결과 : `<div><h2>치킨</h2><h2>피자</h2></div>`
        -> 구분자가 없는 태그로만 구성된 최종데이터를 그자리에 출력함


    const aa = ['치킨','피자','햄버거'];
    -> 배열데이터는 이름인데 뒤에 '씨'붙여서 보관하기로
    aa = aa.map(val=>val+'좋아');
    -> 결과 : ['치킨좋아','피자좋아','햄버거좋아'];
 ...
 ****************************************************/   
    ...
    const bb = aa.map(val=>val+'좋아');

    // console.log(bb);

    // const cc = bb.map(val=>`<div>${val}</div>`);
    const cc = bb.map((val,idx,a)=>`<div>${idx+'.'+val+'/'+a}</div>`);

...
domFn.addEvt(window,'DOMContentLoaded',wheelFn);
...
/************************************************** 
  ...
  [ 자동스크롤 기능정의 ]
  1. 스크롤바가 없는 상태에서 마우스 휠 작동 시 아래와 같이 기능구현 됨
  (1) 휠 내림 : 다음페이지로 이동
  (2) 휠 올림 : 이전페이지로 이동
...
/************************************** 
  ...
  기능: 마우스 휠 작동시 페이지이동
**************************************/
function wheelFn(e){ // e 이벤트 전단별수(자동 - 아무이름 가능인데 보통 e evt 사용)
  // 함수 호출 확인
  // console.log('휠');
  ...
  sts_wheel = 1; // 잠금
  setTimeout(()=>{sts_wheel=0},500); //0.8초 후 잠금 해제
  ...
  // 2. 페이지 이동하기
  // scrollTo(x축위치,y축위치)
  // 세로방향 이동은 두번째값만 주면 된다
  // 스크롤 애니메이션은 CSS로 html{scroll-behavior:smooth}로 처리
...
function touchStart(e){  //e - 이벤트 전달변수
  // 모바일 이벤트 화면 위치값 구하기
  // 모바일 오리리널 이벤트 객체 - originalEvent (제이쿼리에서만 씀)
  ...
  // 함수호출 확인
  // // console.log('터치시작',pos_start, '/결과',result);
...
function touchEnd(e){  //e - 이벤트 전달변수
  ...
  // 제이쿼리는 originalEvent를 사용해야 나옴
  // let scY = e.originalEvent.touches[0].screenY;
  // 터치가 끝날때는 changedTouches를 사용
  pos_end = e.changedTouches[0].screenY;



  // 함수호출 확인
  // console.log('터치끝',pos_end);
...
function movePage(dir){  //dir은 방향값(1- 아래 , 0- 위)
  //함수호출

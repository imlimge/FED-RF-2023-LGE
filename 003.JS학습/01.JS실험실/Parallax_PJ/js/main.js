...
/*******************************************  
  [ 패럴렉스 기능구현하기 ]
  1. 정의 : 패럴렉스란 스크롤 작동 시 같은 방향으로 요소가 다른 속도를 가지고 움직이므로
    사용자가 공강감을 느낄 수 있게 하는 구현 방법

  2. 방법 : 
    (1) 범위 - 요소가 화면에 등장하여 보일동안 스크롤 될 때 이동함
    (2) 움직일 크기 설정이 필요함
...
function moveEl(elPos,ele,setH){
  ...
  /******************************************* 
    ...
    -> Y축 위치 이동은 처음에 0부터 서서히 커지므로 이동 수치값은 전체
    정한범위에서 실제이동값을 빼야함

...
function EventShow(){
  ...
  const aladin = lamp => { //lamp-알라딘 주인공 이미지경로
    ...
    <h1 class="tit">소원이무엇이냐</h1>`;


    //(1-2) 소원이 무엇이냐 타이틀박스 CSS구성
  ...
  const getFerrari = () => {
    ...
    //페라리 이미지 넣기
    //대상: #ferrari
    ReactDOM.render(<MakeImg isrc="https://www.pngplay.com/wp-content/uploads/13/Ferrari-458-Transparent-PNG.png" ialt="페라리레드" info="true" idnm="car" tit="클릭화면 시운전해요" />,
  ...
  return(
    <React.Fragment>
      <div id="tbox" style={{textAlign:"center"}}>
        {/* 소원을 말해봐 이미지 출력
        onmouseover 이벤트적용 */}
        <img src="https://images.chosun.com/resizer/SFIqPKffr3HQHoHFOxKvnN-L2YU=/464x0/smart/cloudfront-ap-northeast-1.images.arcpublishing.com/chosun/NVMCI5M33KHBCY4JVHDPWRCBYY.jpg" alt="지니" onMouseOver={()=>{aladin('./images/ala4.jpg')}}/>
      </div>
      {/* 램프가 들어갈 요소 */}
...
function MakeImg(props){ /// isrc 이미지경로 / ialt 이미지설명
  ...
  //props.info -> ture / false 값을 보내서 처리함
...
function moveCar(){
  ...
  car.style.transform = aaa ?"translateX(150%) scale(2)":"translateX(0) scale(1)";
...
//최초 컴포넌트 출력하기
//ReactDOM.render(어쩌구,저쩌구) 어쩌구를 저쩌구에 출력

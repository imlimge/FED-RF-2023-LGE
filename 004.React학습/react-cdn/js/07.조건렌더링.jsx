...
// 음식리스트
const foods = ["스파게티","짜파게티","냉면","짜장면","마라탕","치킨"];
// const foods = [];
...
//2-3. 개발자 선호 음식 리스트 출력컴포넌트
function WishList(props){ //wlist속성에 담아서 보내준다
  //위시리스트 담기
  ...
  return(
    <React.Fragment>
     ...
     <div>
      ...
      {/* e다른경우의 출력은 별도의 jsx출력 중괄호 구역에 코딩한다 */}
      {
        myFood.length == 0&&
        <h2>아직 개발자 음식 리스트가 없음</h2>
      }
    </React.Fragment>
  );
}/// WishList 컴포넌트 ///
...
// 4-2. 전체출력 컴포넌트 /////
// 구성 : 전체타이틀 (Title 컴포넌트 사용) + 변경버튼
//      + 작가와 그림출력(MakeWork 컴포넌트)
// 특이사항: 변경버튼 클릭 시 MakeWork 컴포넌트의 데이터를 변경하여 다시 출력 (Hook사용)
function ExpComp(props){
  ...
  // -> set변수명 : 변수명 첫글자 대문자
  // useState()메서드가 변수값이 업데이트 되는 여부를 관리하여
  // 변경 시 이 변수를 사용하는 컴포넌트를 자동으로 없데이트 한다
  ...
  const againFn = () => {
    ...
    // 단순히 변수값만 변경한다고 해서 이 변수를 사용하는 컴포넌트를 변경할 수 없다
    // 이런변수의 상태를 관리하는 후크(Hook)를 사용하면 
    // 이것을 반영할 수 있다
  ...
  return(
    <React.Fragment>
      ...
      컴포넌트가 이 변수의 값이 변경됨에 따라 자동으로 재설정 된다*/}
      {
        result ? 

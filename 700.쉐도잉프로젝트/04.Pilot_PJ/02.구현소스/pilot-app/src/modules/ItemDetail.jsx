...
export function ItemDetail({ cat, goods }) {
  ...
  // useRef를 사용한 이유는 리랜더링 시에도 값을 유지하면서 
  ...
  const useCart = () => {
    ...
    // 카트 선택아이템만 추가하기 위해 카트 컴포넌트와 공유한 useRef 참조변수인 flag 값을
    // true로 업데이트 한다
    flag.current = true;
  ...
  return (
    <>
      ...
      // useRef 변수인 flag를 보내면 자식 컴포넌트에서도 이 값을 참조할 뿐만 아니라
      // 변경도 가능하다
      // 주의: useRef 변수는 사용 시 변수명.current를 꼭 쓴다
      }
